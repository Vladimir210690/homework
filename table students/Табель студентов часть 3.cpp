...
void DeliteStudent(students idnumber[], const int SIZE, int & id, int & colpeople) {
	int numberdel = 0;
	for (int i = 0; i < SIZE; i++) {
		cout << idnumber[i].id << " Name: " << idnumber[i].name << " Surname: " << idnumber[i].surname << " Age: " << idnumber[i].age << " Ball: " << idnumber[i].ball << endl;
	}
	cout << "Enter id delete student: " << endl;
	cin >> numberdel;

	for (int i = 0; i < SIZE; i++) {
		if (idnumber[i].id == numberdel) {
			idnumber[i].id = 0;
			idnumber[i].age = 0;
			idnumber[i].ball = 0;
			idnumber[i].name = " ";
			...
			colpeople--;
			for (int i = 0; i < SIZE; i++) {
				if (idnumber[i].id != 0 && idnumber[i].id > numberdel) {
					swap(idnumber[i], idnumber[i - 1]);	
				}
			}
		}
	}
...
int main() {
	...
	do { // Создаем цикл пользования опциями меню, при том что 1 раз можно зайти обязательно, а дальше в зависимости от выбора
		...
		case 2: // Опция удаления студентов (пока нет)
			system("cls");
			DeliteStudent(idnumber, SIZE, id, colpeople);
			break;
